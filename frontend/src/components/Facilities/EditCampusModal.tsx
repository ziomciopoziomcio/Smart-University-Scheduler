// src/components/Facilities/EditCampusModal.tsx
...
interface EditCampusModalProps {
    open: boolean;
    campus: any | null; // Obecnie edytowany kampus
    onClose: () => void;
    onSuccess: () => void;
}
            ...
            <DialogContent sx={{ display: 'flex', flexDirection: 'column', gap: 3, mt: 2 }}>
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1, mt: 1 }}>
                    <Button
                        ...
                        sx={{ py: 1.5, borderRadius: '12px', bgcolor: '#2b5073', textTransform: 'none', fontSize: '1rem', '&:hover': { bgcolor: '#1a3a56' } }}
                    >
                        {isSubmitting ? <CircularProgress size={24} color="inherit" /> : intl.formatMessage({ id: 'facilities.modal.campus.campusEdit.submit' })}
                    </Button>
                    <Button variant="text" fullWidth onClick={onClose} disabled={isSubmitting} sx={{ color: '#2b5073', textTransform: 'none', fontWeight: 600 }}>
