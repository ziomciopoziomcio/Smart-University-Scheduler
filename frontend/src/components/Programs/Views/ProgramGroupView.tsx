                ...
                columns={[
                    {
                        render: (item) => {
                            if (item.major) {
                                ...
                                return <Chip size="small" label={intl.formatMessage({ id: 'programs.groups.chip.major' }, { name: majorName })} color="primary" variant="outlined" />;
                            }
                            if (item.elective_block) {
                                ...
                                return <Chip size="small" label={intl.formatMessage({ id: 'programs.groups.chip.block' }, { name: blockName })} color="secondary" variant="outlined" />;
                            }
                            return <Chip size="small" label={intl.formatMessage({ id: 'programs.groups.chip.general' })} variant="outlined" sx={{ color: 'text.secondary', borderColor: 'divider' }} />;
                        },
                        width: '300px'
                    },
                    {
                        render: (item) => intl.formatMessage({ id: 'programs.groups.studentsCount' }, { count: (item as any).student_count ?? 0 }),
                        variant: 'secondary',
                        width: '150px',
                        align: 'right'
                    }
                ]}
                onItemClick={(item) => navigate(`/programs/faculty/${facultyId}/field/${fieldId}/program/${programId}/semester/${semesterId}/groups/${item.id}`)}
                onMenuOpen={(e, item) => {
