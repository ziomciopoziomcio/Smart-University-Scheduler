...
class ProgramsParser:
    ...
    def __init__(
        ...
        self.main_url = (
            "https://programy.p.lodz.pl/ectslabel-web/?l=pl&wersja202526=true"
        )
    ...
    def get_majors_list(self) -> None:
        ...
        try:
            ...
            with open(
                self.majors_filename, "w", newline="", encoding="utf-8"
            ) as csvfile:
    ...
    def get_majors_all_versions(self, major: str, url: str, soup=None) -> None:
        ...
        safe_major = "".join(
            [c for c in major if c.isalnum() or c in (" ", "-", "_")]
        ).strip()
        ...
        try:
            ...
            wydzial = (
                pre_tag.text.replace("\r", " ").strip()
                if pre_tag
                else "Nieznany Wydział"
            )
            ...
            with open(destination, "w", newline="", encoding="utf-8") as csvfile:
                ...
                for a_tag in soup.find_all("a", onclick=True):
                    ...
                    if match:
                        ...
                        writer.writerow(
                            [name, wydzial, stopien, stacjonarne, full_link]
                        )
        except Exception as e:
    ...
    def parse_major(self, url: str, faculty: str, specialty_name: str = None) -> dict:
        ...
        try:
            ...
            kierunek = {
                ...
                "stopien": (
                    1
                    if "studia pierwszego stopnia" in args_dict.get("stopien", [""])[0]
                    else 2
                ),
        ...
        self.logger.info(
            f"Trying to parse: ({kierunek["stopien"] * "I"} st.) {kierunek['nazwa']} { f"- {specialty_name}" if specialty_name else ""} ({kierunek['od']}) ({"stac." if kierunek['stacjonarne'] else "nstac."})"
        )
        ...
        for table in semester_tables:
            ...
            semestr = {"nazwa": sem_header.text.strip(), "przedmioty": []}
            ...
            for row in tbody.find_all("tr"):
                ...
                przedmiot = {
                    header: item.text.strip() for header, item in zip(headers, items)
                }
                    ...
                    if link_tag and link_tag.has_attr("onclick"):
                        try:
                            ...
                            for tr in karta.find_all("tr"):
                                ...
                                if param and val:
                                    detale[param.get_text(strip=True)] = val.get_text(
                                        " ", strip=True
                                    )
                            ...
                            realizatorzy = [
                                r.strip()
                                for r in realizatorzy_raw.split(",")
                                if r.strip()
                            ]

                            przedmiot.update(
                                {
                                    "jednostka": detale.get(
                                        "Jednostka prowadząca", "Brak danych"
                                    ),
                                    "kierownik": detale.get(
                                        "Kierownik przedmiotu", "Brak danych"
                                    ),
                                    "realizatorzy": realizatorzy,
                                    "jezyk": detale.get(
                                        "Język prowadzenia zajęć", "Brak danych"
                                    ),
                                }
                            )
                        except Exception as e:
                            self.logger.error(
                                f"Error in course page {przedmiot.get('Kod przedmiotu', '???')}: {e}"
                            )

                            file_exists = os.path.isfile(self.missed_filename)
                            with open(
                                self.missed_filename, "a", newline="", encoding="utf-8"
                            ) as m_file:
    ...
    def parse_programs_to_json(self) -> None:
        ...
        if not os.path.exists(self.plans_dir):
            self.logger.warning("Schedules folder does not exist.")
            return
            ...
            if e.is_file() and e.name.endswith(".csv"):
                with open(e.path, "r", encoding="utf-8") as f:
                    ...
                    for p in plans:
                        ...
                        if self.time_between_fos_sec > 0:
                            self.pretty_wait(self.time_between_fos_sec)
                        ...
                        if not spec_dict:
                            ...
                            for spec_id, spec_name in spec_dict.items():
                                data = self.parse_major(
                                    self.parse_link(f"{p[4]}&sp={spec_id}"),
                                    p[1],
        ...
        if self.overwrite:
            self.save_to_json(
                data=kierunki, filename=self.output_filename, label="Preliminary"
            )
    ...
    def retry_missed_subjects(self) -> None:
        ...
        for row in rows:
            ...
            try:
                ...
                new_data = {
                    ...
                    "realizatorzy": [
                        r.strip()
                        for r in detale.get("Realizatorzy przedmiotu", "").split(",")
                        if r.strip()
                    ],
                ...
                for k in kierunki:
                    if (
                        k["nazwa"] == row["nazwa_kierunku"]
                        and k["wydzial"] == row["wydzial"]
                        and str(k["stopien"]) == str(row["stopien"])
                        and str(k["specjalizacja"]) == str(row["specjalizacja"])
                        and k["od"] == row["od"]
                    ):
                ...
                if success:
                    ...
                    self.logger.warning(
                        f"Fetched data but couldn't find matching entry in JSON for: {link}"
                    )
        ...
        self.save_to_json(
            data=kierunki, filename=self.output_filename, label="Filled-in"
        )
        ...
        if not still_missed:
            ...
            with open(
                self.missed_filename, "w", newline="", encoding="utf-8"
            ) as csvfile:
                writer = csv.DictWriter(csvfile, fieldnames=rows[0].keys())
                writer.writeheader()
                writer.writerows(still_missed)
            self.logger.info(
                f"Retry finished. {len(still_missed)} subjects still missing."
            )
    ...
    def raport_and_clean_programs(self):
        ...
        def get_stats(data_list):
            ...
            sub = sum(
                len(sem.get("przedmioty", []))
                for fos in data_list
                for sem in fos.get("semestry", [])
            )
            return m, s, sub

        m_pre, s_pre, sub_pre = get_stats(fields_of_study)
        self.logger.info(
            f"BEFORE: {m_pre} Majors, {s_pre} Semesters, {sub_pre} Subjects"
        )

        cleaned_fos = [
            fos
            for fos in fields_of_study
            if any(len(s.get("przedmioty", [])) > 0 for s in fos.get("semestry", []))
        ]

        removed_fos = [
            [fos.get("nazwa"), fos.get("url")]
            for fos in fields_of_study
            if fos not in cleaned_fos
        ]
        ...
        self.logger.info(
            f"AFTER: {m_post} Majors, {s_post} Semesters, {sub_post} Subjects"
        )
