...
def test_endpoint_view_elective_blocks(
    client, db_session, get_auth_headers, role_name, expected_status
):
    headers = get_auth_headers(role_name)
    response = client.get("/course/elective-blocks", headers=headers)
...
def test_endpoint_view_elective_block(
    client,
    db_session,
    get_auth_headers,
    ...
    headers = get_auth_headers(role_name)
    block = create_test_elective_block(block_name=f"Block_{role_name}")

    response = client.get(f"/course/elective-blocks/{block.id}", headers=headers)

    assert response.status_code == expected_status
    if expected_status == 200:
        data = response.json()
        assert data["elective_block_name"] == block.elective_block_name
...
def test_endpoint_create_elective_block(
    client,
    db_session,
    get_auth_headers,
    ...
    headers = get_auth_headers(role_name)
    field = create_test_study_field(field_name=f"Field_for_Block_{role_name}")

    payload = {"study_field": field.id, "elective_block_name": f"New_Block_{role_name}"}

    response = client.post("/course/elective-blocks", json=payload, headers=headers)

    assert response.status_code == expected_status
    if expected_status == 201:
        assert response.json()["elective_block_name"] == payload["elective_block_name"]
...
def test_endpoint_update_elective_block(
    client,
    db_session,
    get_auth_headers,
    create_test_elective_block,
    ...
    headers = get_auth_headers(role_name)
    block = create_test_elective_block(block_name=f"Old_Block_{role_name}")

    payload = {"elective_block_name": f"Updated_Block_{role_name}"}

    response = client.patch(
        f"/course/elective-blocks/{block.id}", json=payload, headers=headers
    )
...
def test_endpoint_delete_elective_block(
    client,
    db_session,
    get_auth_headers,
    create_test_elective_block,
    ...
    headers = get_auth_headers(role_name)
    block = create_test_elective_block(block_name=f"To_Delete_{role_name}")

    response = client.delete(f"/course/elective-blocks/{block.id}", headers=headers)

    assert response.status_code == expected_status
    if expected_status == 204:
        admin_headers = get_auth_headers("Administrator")
        check = client.get(f"/course/elective-blocks/{block.id}", headers=admin_headers)
